...
const NextArrow: React.FC<ArrowProps> = ({ onClick }) => (
  <button 
    onClick={onClick}
    className="absolute right-2 sm:right-4 top-1/2 -translate-y-1/2 z-10 w-8 h-8 sm:w-10 sm:h-10 flex items-center justify-center bg-black/40 hover:bg-black/60 text-white text-xl sm:text-2xl font-light rounded-full transition-colors duration-200 hidden md:flex"
    type="button"
...
const PrevArrow: React.FC<ArrowProps> = ({ onClick }) => (
  <button 
    onClick={onClick}
    className="absolute left-2 sm:left-4 top-1/2 -translate-y-1/2 z-10 w-8 h-8 sm:w-10 sm:h-10 flex items-center justify-center bg-black/40 hover:bg-black/60 text-white text-xl sm:text-2xl font-light rounded-full transition-colors duration-200 hidden md:flex"
    type="button"
...
const WarehouseDetails: React.FC = () => {
  ...
  // Process images function with improved debugging
  const processImages = useCallback(
    (images?: string | string[] | null): string[] => {
      ...
      if (processedImages.length === 0) {
        processedImages = [`${baseUrl}/uploads/default.jpg`];
      }
      ...
      try {
        ...
        if (axios.isAxiosError(error)) {
          if (error.response?.status === 404) {
            setError('Warehouse not found or not available for public viewing');
          } else {
            setError(`Error loading warehouse: ${error.response?.data?.message || error.message}`);
          }
        } else {
  ...
  // Slider settings with fixed arrows
  const sliderSettings = {
    ...
    customPaging: () => (
      <button 
        className="w-2 h-2 sm:w-3 sm:h-3 mx-1 rounded-full bg-white/60 hover:bg-white transition-colors duration-200 border-0 outline-none cursor-pointer" 
        type="button"
        style={{ outline: 'none' }}
      />
    ),
                    ...
                    <div key={index} className="outline-none">
                      <div className="relative w-full h-0 pb-[56.25%]"> {/* 16:9 Aspect Ratio */}
                        <img
                          ...
                          alt={`${warehouse?.name || 'Warehouse'} view ${index + 1}`}
